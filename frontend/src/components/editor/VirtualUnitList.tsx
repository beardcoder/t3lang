...
interface VirtualUnitListProps {
  ...
  readonly isSourceOnly: boolean;
  readonly onDeleteUnit: (unitId: string) => void;
}

const ROW_HEIGHT = 72;
const HEADER_HEIGHT = 42;

export function VirtualUnitList({ units, filePath, isSourceOnly, onDeleteUnit }: Readonly<VirtualUnitListProps>) {
  ...
  const moveFocus = useCallback(
    (fromUnitId: string, direction: 'up' | 'down') => {
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('unit-tab-navigate', handleTabNavigate);

    return () => globalThis.removeEventListener('unit-tab-navigate', handleTabNavigate);
  }, [moveFocus]);
  ...
  if (units.length === 0) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="rounded-full bg-bg-tertiary px-3 py-1.5 text-sm text-text-tertiary">No translation units found</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="relative w-full" style={{ height: `${virtualizer.getTotalSize() + HEADER_HEIGHT}px` }}>
        <div className="sticky top-0 z-20 flex text-[11px] font-semibold uppercase tracking-wider text-text-tertiary">
          <div className="w-1/4 min-w-37.5 px-3 py-3">Key</div>
