...
export function ContextBar() {
  ...
  if (activeGroup && viewMode !== "dashboard") {
    ...
    if (activeLanguage && viewMode === "editor") {
      const languageLabel =
        activeLanguage === "default" ? "Source" : activeLanguage.toUpperCase();
        ...
        {breadcrumbs.length === 0 ? (
          ...
          breadcrumbs.map((crumb, index) => (
            <span key={index} className="flex min-w-0 items-center gap-1">
              {index > 0 && (
                <ChevronRight className="h-3.5 w-3.5 text-text-tertiary/75" />
              )}
              <span
                className={`rounded px-1 py-0.5 ${
                  crumb.onClick
                    ? "cursor-pointer rounded-full px-2.5 py-1 text-text-primary hover:bg-bg-tertiary"
                    : index === breadcrumbs.length - 1
                      ? "rounded-full bg-accent-light px-2.5 py-1 font-medium text-accent"
                      : "max-w-[11rem] truncate rounded-full bg-bg-tertiary/70 px-2.5 py-1 text-text-tertiary"
                }`}
