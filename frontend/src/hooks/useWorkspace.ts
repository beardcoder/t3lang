...
import { useWorkspaceStore, useEditorStore, usePersistenceStore, useUIStore } from '../stores';
import { ScanWorkspace, ReadTextFile, WriteFileAtomic, StartWatching, StopWatching } from '../../wailsjs/go/main/App';
...
export function useWorkspace() {
  ...
  const loadFile = useCallback(
    async (filePath: string): Promise<FileData | null> => {
      // Check cache first
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('open-workspace', handleOpenWorkspace as EventListener);

    return () => {
      globalThis.removeEventListener('open-workspace', handleOpenWorkspace as EventListener);
    };
  }, [openWorkspace]);
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('save-all', handleSaveAll);

    return () => globalThis.removeEventListener('save-all', handleSaveAll);
  }, [saveAllFiles]);
